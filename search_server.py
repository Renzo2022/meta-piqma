...
def generate_forest_plot_graph_svg(extraction_data: list, i2: float, q: float, p_value: float, combined_effect: float) -> str:
    ...
    # Márgenes para centrar el gráfico - aumentado significativamente para evitar cortes
    margin_left = 120
    margin_right = 100
    graph_width = 1000
    graph_start = margin_left
    total_width = graph_start + graph_width + margin_right
...
def generate_funnel_plot_graph_svg(extraction_data: list, i2: float, q: float, p_value: float) -> str:
    ...
    svg = f'''<svg width="{total_width}" height="800" xmlns="http://www.w3.org/2000/svg">
    <!-- Fondo -->
    <rect width="{total_width}" height="800" fill="#f8f9fa"/>
    ...
    <!-- Línea de referencia (efecto nulo) -->
    <line x1="{graph_start + graph_width//2}" y1="100" x2="{graph_start + graph_width//2}" y2="650" stroke="#999" stroke-width="2" stroke-dasharray="5,5"/>
    <text x="{graph_start + graph_width//2}" y="90" font-size="10" text-anchor="middle" fill="#666">No Effect</text>
    
    <!-- Líneas de confianza (95%) -->
    <line x1="{graph_start + graph_width//2 - 200}" y1="100" x2="{graph_start + graph_width//2}" y2="650" stroke="#E0E0E0" stroke-width="1" stroke-dasharray="3,3"/>
    <line x1="{graph_start + graph_width//2 + 200}" y1="100" x2="{graph_start + graph_width//2}" y2="650" stroke="#E0E0E0" stroke-width="1" stroke-dasharray="3,3"/>
    
    <!-- Estudios con etiquetas -->'''
    
    for i, study in enumerate(studies):
        x = graph_start + graph_width//2 + (study['effect'] - 1.5) * 200
        y = 650 - (study['se'] - 0.05) * 1800
        color = study.get('color', '#2196F3')
    ...
    svg += f'''
    <!-- Etiquetas de ejes -->
    <text x="{graph_start + graph_width//2}" y="730" font-size="12" text-anchor="middle" fill="#333">Effect Size</text>
    <text x="20" y="400" font-size="12" text-anchor="middle" fill="#333" transform="rotate(-90 20 400)">Standard Error</text>
    
    <!-- Escala X -->
    <text x="{graph_start}" y="730" font-size="10" text-anchor="middle" fill="#333">0.5</text>
    <text x="{graph_start + graph_width//6}" y="730" font-size="10" text-anchor="middle" fill="#333">1.0</text>
    <text x="{graph_start + graph_width//3}" y="730" font-size="10" text-anchor="middle" fill="#333">1.5</text>
    <text x="{graph_start + graph_width//2}" y="730" font-size="10" text-anchor="middle" fill="#333">2.0</text>
    <text x="{graph_start + 2*graph_width//3}" y="730" font-size="10" text-anchor="middle" fill="#333">2.5</text>
    
    <!-- Escala Y -->
    <text x="{graph_start - 40}" y="660" font-size="9" text-anchor="end" fill="#333">0.05</text>
    <text x="{graph_start - 40}" y="480" font-size="9" text-anchor="end" fill="#333">0.15</text>
    <text x="{graph_start - 40}" y="300" font-size="9" text-anchor="end" fill="#333">0.25</text>
    
    <!-- Leyenda -->
    <rect x="{graph_start + graph_width + 60}" y="100" width="230" height="100" fill="white" stroke="#ccc" stroke-width="1" rx="5"/>
    <text x="{graph_start + graph_width + 75}" y="120" font-size="11" font-weight="bold" fill="#333">Interpretation:</text>
    <text x="{graph_start + graph_width + 75}" y="140" font-size="10" fill="#666">• Symmetric: No bias</text>
    <text x="{graph_start + graph_width + 75}" y="155" font-size="10" fill="#666">• Asymmetric: Possible bias</text>
    <text x="{graph_start + graph_width + 75}" y="170" font-size="10" fill="#666">• I² = {i2}%: Heterogeneity</text>
    <text x="{graph_start + graph_width + 75}" y="185" font-size="10" fill="#666">• p-value = {p_value}</text>
